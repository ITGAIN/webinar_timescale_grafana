...
select create_hypertable('observability.hist_pg_stat_database', 'time', migrate_data => TRUE, chunk_time_interval => INTERVAL '1 day', if_not_exists => TRUE);
...
SELECT add_compression_policy('observability.hist_pg_stat_database', INTERVAL '1h');
SELECT add_retention_policy('observability.hist_pg_stat_database', INTERVAL '7d');

/* bash
while true; do docker compose exec -t timescale psql -U postgres -d postgres -c "insert into observability.hist_pg_stat_database select current_timestamp as time,* from pg_stat_database;"; sleep 60;done
*/

select "time",numbackends from observability.hist_pg_stat_database